...
const ReviewCodeOutputSchema = z.object({
  issues: z.array(IssueSchema).describe('A prioritized list of issues and code suggestions. If no issues are found, this should be an empty array.'),
});
...
Analyze the following code changes and identify relevant issues. For each issue, provide a priority, a clear description, and a specific code suggestion for how to fix it. Be critical and thorough.
...
Focus your entire review on **{{{focus}}}**. If no issues are found, return an empty array for "issues".
